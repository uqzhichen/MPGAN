...
class LoadDataset(object):
    def __init__(self, opt):
        ...
        self.mean_inter_dist = np.zeros([train_cls_num, self.part_num]).astype(np.float32)
        self.nearest_intra_dist = np.zeros([train_cls_num, self.part_num]).astype(np.float32)
